...
for ($i = 2; $i < $filesCount; $i++) {
    ...
    $events = getEvents($session, $start_date_obj, $end_date_obj, 14);
    
    // Выборка событий перехода на определённый статус
    $events = array_filter($events, function($event) use($settings) {
        return in_array($event['value_after']['status_id'], $settings['statuses']);
    });

    // Получение ID сделок из событий
    $leadsIds = [];
    ...
    // Получение списка событий изменения поля сделки за период
    $events = getEvents($session, $start_date_obj, $end_date_obj, 24);
    ...
    // Получение списка событий смены ответственного за период сделки за период
    $events = getEvents($session, $start_date_obj, $end_date_obj, 25);
    ...
    $dropboxNotes = getDboxNotes($session, $start_date_obj);
    ...
    $leadsIds = array_merge($leadsIds, array_column($periodicalDropboxNotes, 'element_id'));
    $leadsIds = array_unique($leadsIds);
    ...
    // Выборка сделок на определённых статусах
    $leads = array_filter($leads, function($lead) use($settings) {
        return in_array($lead['status_id'], $settings['statuses']);
    });
    ...
    if (count($leads) > 0) {
        ...
        // Массив контактов
        $contacts = getContactsByIds($session, $cids);
