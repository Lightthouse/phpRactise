...
function getUsers($session, $offset = 0)
{
    // Адрес запроса
    $url = sprintf("account?with=users&limit_offset=%s&limit_rows=500", $offset);
    ...
    if (count($response['_embedded']['users']) < 500)
    {
        // Выход из рекурсии
    ...
    return array_merge($response['_embedded']['users'], getUsers($session, $offset));
}
?>
